...
// with the column in the database "users" table?
type User struct {
	ID             int
	Name           string
	Email          string
...
func (m *UserModel) Insert(name, email, password string) error {
	// Create a bcrypt hash of the plain-test password.
	...
	if err != nil {
		...
		// if the error code equals to 1062 and contents of the error message string.
		// If it does, we return ErrDuplicateEmail error.
...
// We'll use Authenticate method to verify whether a user exists with
